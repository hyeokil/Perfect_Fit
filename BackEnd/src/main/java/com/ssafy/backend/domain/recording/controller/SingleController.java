...
public class SingleController {
    ...
    // 단일 조회
    @GetMapping("/get/{singleId}")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<Message<SingleResponseDto>> getSingle(@PathVariable("singleId") Long singleId) {
        SingleResponseDto single = singleService.getSingle(singleId);
        return ResponseEntity.ok().body(Message.success(single));
    }
